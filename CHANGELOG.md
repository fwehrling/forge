...
- **Skill descriptions clarified and hub disambiguation rules added**: applied validated best-descriptions to `forge-init`, `forge-loop`, `forge-update`. Tightened exclusive signals for `forge-debug` (UNKNOWN cause only, excludes hotfix/one-liner phrasing) and `forge-quick-spec` (already-identified cause only, excludes root-cause/investigation phrasing). Made `forge-auto` sequential-only explicit and point to `forge-team` for parallel execution. Added 4 disambiguation rules in the hub for the memory/wiki and party/team clusters. No functional change to any skill; only frontmatter descriptions and hub routing hints.
...
- **forge-verify transient statuses**: the QA satellite was writing transient values like `fix-applied` into `.forge/sprint-status.yaml` without ever transitioning back to a canonical state, desyncing `/forge status`, `/forge resume`, and the hub flow state. `skills/forge-verify/SKILL.md` step 9 now defines the canonical vocabulary (`pending` | `in_progress` | `blocked` | `completed`) and the verdict mapping (`PASS`/`CONCERNS` -> `completed`, `FAIL` -> `in_progress`), and explicitly forbids transient labels. A fix iteration is just another Dev cycle followed by a fresh verify run -- the final write must be one of the four canonical values.
...
- **forge-update scope cleanup**: removed the "Retrofit wiki vault" step from `/forge-update`. Updating the framework installation (hub + satellites + hooks in the user home) and initializing a project (`.forge/`, wiki vault, memory) are now strictly separated. Legacy projects missing `.forge/wiki/` must run `/forge-init` explicitly instead of relying on `/forge-update` as a side effect. The `--full` flag was also dropped from `/forge-update`.
...
- **forge-init**: add forge-memory CLI installation step (step 9) before memory logging -- ensures forge-memory is available before first use
- **forge-hooks-setup.sh**: correct forge-slim path from `~/.claude/skills/` to `~/.forge/skills/` to match hub-only architecture
...
- `update.sh`: sync `rtk-native-hook.sh` to `~/.claude/hooks/` on update if RTK was previously installed (was missing since v1.9.7)
- `update.sh`: call `inject-rtk-claude-md.sh` on update to keep the RTK block in `~/.claude/CLAUDE.md` current

## [1.9.9] - 2026-04-06

### Changed

- All SKILL.md files translated to English for international accessibility (9 files: forge hub, forge-update, forge-memory, and all 6 Business Pack skills)
- forge hub: "French Language Rule" replaced with "Language Rule" -- Claude now responds in the user's language automatically
- Business Pack personas (Clara, Theo, Maya, Victor, Leo) and their full instructions rewritten in English
- forge-legal: instructions in English, French legal acronyms (RGPD, CGV, LCEN, URSSAF) preserved as proper nouns

## [1.9.8] - 2026-04-06

### Changed

- README: agent count corrected 11 -> 12 (add DevOps, replace incorrect Debug entry, mention Quick-QA variant)
- README: hooks list updated -- add `forge-skill-tracker.sh` (was missing)
...
- statusline user customization hook: sources `~/.claude/hooks/statusline-custom.sh` before output, exposing all internal vars (`MODEL`, `FORGE_MARKER`, `SKILL_INDICATOR`, `PROJECT`, `CTX`, `PARTS`, `EFFORT_LABEL`). Changes survive FORGE updates

## [1.9.4] - 2026-04-01

### Fixed

- remove UserPromptSubmit hook and .claude/ from repo
## [1.9.3] - 2026-04-01

### Fixed

- move release/ship scripts to .claude/scripts (gitignored)
## [1.9.2] - 2026-04-01

### Fixed

- create GitHub Release on ship, not just push tag (#61)
## [1.9.1] - 2026-04-01

### Fixed

- prevent false major bump from commit body prose (#60)
- smarter version bump logic in release script (#59)
## [1.9.0] - 2026-04-01

### Added

- add UserPromptSubmit hook for auto-routing and fix skill tracker persistence (#58)
## [1.8.0] - 2026-04-01
...
- **forge-memory setup.sh**: use disk-backed TMPDIR for pip installs instead of /tmp (tmpfs 512 Mo on Hostinger VPS was too small for numpy/sentence-transformers downloads)
...
- **Version in status line**: when `.forge/` is detected, the status line now shows `FORGE v1.x.x` instead of just `FORGE`
...
- **Nested skill directories on Linux**: `cp -r dir/ target/` on Linux creates `target/dir/` inside an existing `target/`, producing `forge-build/forge-build/SKILL.md`. Fix: remove target directory before copying in both `install.sh` (business pack) and `update.sh` (all skills). Root cause of all false-positive "modified" reports on Linux
...
- **Shell compatibility**: replace all `echo -e` with `printf '%b'` in `install.sh` and `update.sh` for POSIX `sh` compatibility (fixes `-e` prefix in output when run with `sh` instead of `bash`)
...
- **Skill prompt optimization**: streamlined 19 SKILL.md files, removing verbosity, hardcoded commands, and redundant instructions (-240 lines net). Added disambiguation table to the router
- **gitignore**: added `.gitignore` to exclude workspace artifacts and `.env` secrets

## [1.7.8] - 2026-03-28

### Fixed

- **VERSION file**: was stuck at 1.7.3 since v1.7.4 -- now correctly reads 1.7.8
...
[1.12.6]: https://github.com/fwehrling/forge/compare/v1.12.5...v1.12.6
