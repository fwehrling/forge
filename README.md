...
**v1.14.3**: skill descriptions clarified and hub disambiguation rules added. Applied validated best-descriptions to `forge-init`, `forge-loop`, `forge-update`. Tightened exclusive signals for `forge-debug` (UNKNOWN cause only, excludes hotfix/one-liner phrasing) and `forge-quick-spec` (already-identified cause only, excludes root-cause/investigation phrasing). Made `forge-auto` sequential-only explicit and point to `forge-team` for parallel execution. Added 4 disambiguation rules in the hub for the memory/wiki and party/team clusters. No functional change to any skill; only frontmatter descriptions and hub routing hints.

**v1.14.2**: prod release auto-sync to flow-state. `forge-memory-sync.sh` Stop hook gains a Stage 3 that detects release bumps (`chore(release): vX.Y.Z` commit or `v*` tag at HEAD) and queues `.forge/flow-state-pending.yaml`. The hub drains this queue at Startup Protocol step 0b and updates `flow-state.yaml` `prod_state` (backend_version, last_release_commit, release_history capped at 10). Fixes the drift when releases shipped outside the FORGE pipeline left the hub pointing at an old version. Stage 3 runs independently of Stage 2 -- projects without `.forge/wiki/` still get release state persisted. Startup Protocol drains (pending wiki ingest + flow-state) are now mandatory first-step before any response.
